...
function OutLink({
  ...
  return (
    <a
      ...
      className="inline-flex items-center gap-0.5 font-medium text-ceyfi-deep dark:text-[#34D399] underline-offset-4 hover:text-ceyfi-green hover:underline"
    >
      {label}
...
export function SeylanBankHandoffBanner() {
  return (
    <div
      className="border-t border-ceyfi-line dark:border-white/[0.06] bg-white/85 dark:bg-[rgba(12,4,7,0.92)] px-4 py-4 text-xs text-muted-foreground dark:text-white/35 shadow-[0_-12px_32px_-16px_rgba(36,19,22,0.08)] dark:shadow-[0_-1px_0_rgba(255,255,255,0.04)] backdrop-blur sm:px-6"
      aria-label="Official Seylan Bank channels when actions are beyond this demo"
    >
      <p className="mb-3 max-w-3xl leading-relaxed">
        CEYFI is a guided experience layer. Payments, mandates, beneficiary
      ...
      <nav className="flex flex-wrap items-center gap-x-4 gap-y-2 border-t border-ceyfi-line/60 dark:border-white/[0.06] pt-3">
        <OutLink href={SEYLAN_LINKS.internetBankingPersonalLogin} label="Personal Internet Banking" />
        <span className="hidden text-ceyfi-line dark:text-white/10 sm:inline">|</span>
      ...
      <p className="mt-3 text-[11px] leading-relaxed opacity-90">
        ...
        <a
          ...
          target="_blank"
          rel={EXTERNAL_LINK_REL}
          className="font-medium text-ceyfi-deep dark:text-[#34D399] underline-offset-4 hover:text-ceyfi-green hover:underline"
        >

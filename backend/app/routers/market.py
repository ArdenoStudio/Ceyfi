...
def _unwrap_items(payload: Any) -> list[Any] | None:
...
async def market_overview(authorization: str | None = Header(default=None)):
    ...
    live_watch = _unwrap_items(live)
    if live_watch is not None:
        watch = live_watch
        source = "chime"

    live_fires = await _chime_get("/api/v1/alerts/history?limit=10")
    fire_items = _unwrap_items(live_fires)
    if fire_items is not None:
        fires = fire_items
        source = "chime"
...
async def market_watchlist(authorization: str | None = Header(default=None)):
    uid = _user_from_auth(authorization)
    live_items = _unwrap_items(await _chime_get("/api/v1/watchlist"))
    if live_items is not None:
        return {"source": "chime", "nfa": NFA, "items": live_items}
    return {"source": "mock", "nfa": NFA, "items": _MOCK_WATCH.get(uid, [])}


@router.get("/alerts")
async def market_alerts(authorization: str | None = Header(default=None)):
    uid = _user_from_auth(authorization)
    live_items = _unwrap_items(await _chime_get("/api/v1/alerts"))
    if live_items is not None:
        return {"source": "chime", "nfa": NFA, "items": live_items}
    return {"source": "mock", "nfa": NFA, "items": _MOCK_ALERTS.get(uid, [])}
...
async def market_fires(authorization: str | None = Header(default=None)):
    ...
    live_items = _unwrap_items(live)
    if live_items is not None:
        return {"source": "chime", "nfa": NFA, "items": live_items}
    return {"source": "mock", "nfa": NFA, "items": _MOCK_FIRES.get(uid, [])}
